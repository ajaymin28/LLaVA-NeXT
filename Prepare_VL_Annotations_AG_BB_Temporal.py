...
def parse_arguments():
    ...
    parser.add_argument(
        "--output_json_dir",
        type=str,
        default="/home/jbhol/dso/gits/ActionGenome/AG_llava_annotations_v5_3_with_bb_temporal_vidnorm",
    ...
    for video_id, video_frame_block_data in tqdm(overall_annotations):
        ...
        for frame_id, frame_triplets,frame_triplets_bb in video_frame_block_data:
            ...
            for trip_idx, triplet in enumerate(frame_triplets):
                ...
                # import pdb
                # pdb.set_trace()
                FRAME_NORM_HEIGHT = frame_h
                FRAME_NORM_WIDTH = frame_w
                ...
                if s not in added_object_entities:
                    if len(sub_bb)>0:
                        ...
                        bounding_box_string[f"frame-{frame_counter}"].append({s : sub_bb})
                ...
                if o not in added_object_entities:
                    if len(obj_bb)>0:
                        if f"frame-{frame_counter}" not in bounding_box_string.keys():
                            bounding_box_string[f"frame-{frame_counter}"] = []
                        bounding_box_string[f"frame-{frame_counter}"].append({o : obj_bb})
