...
def parse_arguments():
    ...
    parser.add_argument(
        "--output_json_dir",
        type=str,
        default="/home/jbhol/dso/gits/ActionGenome/AG_llava_annotations_v5_3_with_temporal_uprompt",
...
if __name__=="__main__":
    ...
    for video_id, video_frame_block_data in tqdm(overall_annotations):
        annotation_string = ""
        added_frame_ids = []
        triplet_active_spans = defaultdict(list)
        current_active = {}
        ...
        for frame_id, frame_triplets in video_frame_block_data:
            ...
            current_triplets = set(tuple(t) for t in frame_triplets)  # Normalize as tuple keys


            # Finalize spans for triplets no longer active
            ended_triplets = set(current_active.keys()) - current_triplets
            for triplet in ended_triplets:
                start_idx = current_active[triplet]
                end_idx = max(0,len(added_frame_ids)-1)
                triplet_active_spans[triplet].append([start_idx, end_idx])
                del current_active[triplet]

            # Start new spans or continue active ones
            for triplet in current_triplets:
                if triplet not in current_active:
                    current_active[triplet] = len(added_frame_ids)
            ...
            if len(added_frame_ids)>=8:
                ...
                # Finalize any remaining active triplets at the last frame
                for triplet, start_idx in current_active.items():

                    if not triplet_active_spans[triplet] or triplet_active_spans[triplet][-1][1] < 7:
                        triplet_active_spans[triplet].append([start_idx, 7])

                # Flatten spans with their triplet info
                span_triplets = []
                for triplet, spans in triplet_active_spans.items():
                    for span in spans:
                        span_triplets.append((span[0], span[1], triplet))

                # Sort by start time
                span_triplets = list(set(span_triplets))
                span_triplets.sort(key=lambda x: x[0])
                
                # put it togather
                for start, end, (s, p, o) in span_triplets:
                    annotation_string += f"[{s}:{p}:{o}]_[Frame-{start}:Frame-{end}];"

                PromptAnswer = getPromptTemplate(media_path=video_path,media_type="video")

                add_video_token = True

                AG_Prompt = getRandomPrompt(key='AG_Prompt_Temporal', static=True)
