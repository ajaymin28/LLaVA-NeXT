...
def prepare_vid_sg_threaded(data_root,subset, annotations,norm_bb=True, dataset="vidor", uniform_sampling_idx=8, dataset_meta=None):
	...
	for annot_idx, annot in enumerate(annotations):
		...
		for rel in rels:
			...
			for activity_range in range(begin_fid,end_fid):
				...
				if activity_range>frame_count:
					continue
				# if type(total_frames_list[activity_range])==int:
				# 	total_frames_list[activity_range] = []

				# if type(total_frames_list_bb[activity_range])==int:
				# 	total_frames_list_bb[activity_range] = []
				...
				# total_frames_list[activity_range].append([f"{subj_data['category']}-{subj_data['tid']}", predicate, f"{obj_data['category']}-{obj_data['tid']}"])
				# total_frames_list_bb[activity_range].append([sub_bb, obj_bb])

		
		# total_frames_list = [i for i in total_frames_list if type(i)==list]
		# total_frames_list_bb = [i for i in total_frames_list if type(i)==list]
		...
		if os.path.exists(video_path):
			...
			for frame_idx, frame_data in frames_dict.items():
				...
				for index_to_draw, triplet in enumerate(frame_data["triplets"]):
					# triplet = total_frames_list[frame_idx][index_to_draw]
					# print(triplet)
					# bboxes = total_frames_list_bb[frame_idx][index_to_draw]
											
					# bboxes = frame_data["bbox"][index_to_draw]
		...
		for overall_annot in overall_annotations:
			...
			## TODO remanining annotations which are less than 1000 is not saved here.
