...
New: Time Block is added at the end of the trilplet e.g. [dog,eating,food]_[Frame-0,Frame-20]
	...
	for annot_idx, annot in enumerate(annotations):
		...
		for selected_frames in frameblocks:
			...
			for frame_blk_idx, frame_idx in enumerate(selected_frames):
				...
				for index_to_draw, triplet in enumerate(frame_data["triplets"]):				
					...
					if construct_triplet not in triplets_and_time_window.keys():
						...
						triplets_and_time_window[construct_triplet]["timewindows"] = [{"start": frame_blk_idx,
							"end": frame_blk_idx
						}]
					else:
						if triplets_and_time_window[construct_triplet]["timewindowended"]==True:
							triplets_and_time_window[construct_triplet]["timewindows"].append({"start": frame_blk_idx,"end": frame_blk_idx})
							triplets_and_time_window[construct_triplet]["timewindowended"]=False
						else:
							# print("updating end window", frame_blk_idx)
							if frame_blk_idx<triplets_and_time_window[construct_triplet]["timewindows"][-1]["end"]:
								triplets_and_time_window[construct_triplet]["timewindows"][-1]["end"] = frame_blk_idx
							else:
								triplets_and_time_window[construct_triplet]["timewindows"][-1]["end"] = frame_blk_idx
				...
				for triplet, tripletData in triplets_and_time_window.items():
					if not tripletData["updated_in_this_frame"]:
						triplets_and_time_window[triplet]["timewindowended"] = True
						triplets_and_time_window[triplet]["timewindows"][-1]["end"] =  frame_blk_idx-1 # mark prev frame as end for this triplet
			...
			if current_block_triplet_length>0:
				...
				PromptAnswer["conversations"].append(convQ)
				PromptAnswer["conversations"].append(convA)
				PromptAnswer["frame_indices"] =  selected_frames
