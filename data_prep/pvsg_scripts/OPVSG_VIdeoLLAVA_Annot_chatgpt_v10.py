...
def getListofCategoryString(vid_objects, vid_data, addObjectId=False, addFrames=False, addBB=False , uniform_sampling_idx=8):
    ...
    for vid_obj_idx, vobj in enumerate(vid_objects):
      ...
      if addBB:
        ...
        for frame_for_bb_idx in range(frame_start, frame_end+1):
          for uni_sample_idx, uniform_sampled_frame_idx in enumerate(every8thIndex):
              cur_dist = abs(frame_for_bb_idx-uniform_sampled_frame_idx)
              if min_dist is None:
                min_dist = cur_dist
                min_dist_idx = uni_sample_idx
                # best_frame = frame_for_bb_idx
              elif cur_dist<min_dist:
                min_dist = cur_dist
                min_dist_idx = uni_sample_idx
                # best_frame = frame_for_bb_idx
        try:
          sub_bb, mask_size = getboundingBoxOftheObject(data_root=data_root,vid_id=vid_data["video_id"],frame_id=min_dist_idx, object_id=object_id)
      ...
      if addFrames:
        AnswerString += f"_[{min_dist_idx}]"  # index is fixed from 0 to 7 everytime, irrespective of total frames since video-llava takes only 8 frames

      if vid_obj_idx!=len(vid_objects)-1:
        AnswerString +=","

    return AnswerString

def getListofCategoryString_old(vid_objects, vid_data, addObjectId=False, addFrames=False, addBB=False , uniform_sampling_idx=8):
...
def prepare_image_sg_old(chunk_vid_data_keys,data, norm_bb=True, dataset="vidor", uniform_sampling_idx=8):
  ...
  for vid_data_key in chunk_vid_data_keys:
    ...
    for frame_idx in range(min_frame_idx, max_frame_idx+1):
      ...
      PromptAnswer = getPromptTemplate(media_path=image_path,media_type="image")


      PromptAnswer["conversations"].append(getConvBlock(value=getRandomPrompt(key='identify_subject_objects', static=True),
                                                        conv_type="human",
                                                        media_type="image", add_media_token=True))
      PromptAnswer["conversations"].append(getConvBlock(value=ListOfObjectsAnswerStr_withbb,
                                                        conv_type="gpt",
                                                        media_type="image"))
      

      PromptAnswer["conversations"].append(getConvBlock(value=getRandomPrompt(key='identify_predicates', static=True),
                                                        conv_type="human",
                                                        media_type="image", add_media_token=True))
      PromptAnswer["conversations"].append(getConvBlock(value=AnswerString,
                                                        conv_type="gpt",
                                                        media_type="image"))
...
def prepare_vid_sg_threaded(chunk_vid_data_keys,data, norm_bb=True, dataset="vidor", uniform_sampling_idx=8):
   ...
   for vid_id in chunk_vid_data_keys:
      ...
      convA = getConvBlock(value=getListofCategoryString(vid_objects, vid_data, addObjectId=True,addBB=True,addFrames=False), 
                          conv_type="gpt", media_type="<image>", 
      ...
      convQ = getConvBlock(value=getRandomPrompt(key='identify_predicates', static=True), 
                          ...
                          add_media_token=False)
      convA = getConvBlock(value=getObjectsRelations(vid_data["relations"], vid_data, uniform_sampling_idx=uniform_sampling_idx), 
                          conv_type="gpt", media_type="<image>", 
                          add_media_token=False)
...
def getObjectsRelations(vid_rels, vid_data, norm_frames=True, add_frames=True, uniform_sampling_idx=8):
    ...
    for idx, vid_r in enumerate(vid_rels):
      ...
      # for current rel get frame thats near or equal to uni-sampled index
      ...
      sub_bb, obj_bb, mask_size = get_bb_subj_obj(data_root=data_root,vid_id=vid_id,frame_idx=min_dist_idx,subject_id=sub,object_id=obj)
      ...
      if [sub,rel,obj] not in SubObjRel:
          if not add_frames:
            ...
            AnswerString += f"[{vid_objects_by_id[sub]['category']}-{sub}-{sub_bb}:{rel}:{vid_objects_by_id[obj]['category']}-{obj}-{obj_bb}_[{min_dist_idx}]]"
...
def getObjectsRelations_old(vid_rels, vid_data, norm_frames=True, add_frames=True, uniform_sampling_idx=8):
    """
    uniform_sampling_idx=8 for Video-LLAVA
...
if __name__=="__main__":
    ...
    OUTPUT_JSON_DIR = "/lustre/fs1/home/jbhol/dso/gits/OpenPVSG/data/video_llava_annotations_v10/"
