...
def addObjectsRelations_bb_instructions(video_path,vid_data,total_frames, subjobj_rel_frames_data, frame_indices,bb_per_object):
  ...
  for frame_list_idx, frame_idx in enumerate(frame_indices):
    ...
    for rel_idx, relation in enumerate(subj_obj_rel):
      ...
      if bb_per_object[subject_category_name]<50 or bb_per_object[object_category_name]<50:
        ...
        # AnswerString_rel_bb += f"""{subject_category_name}-{sub}:{rel}:"""
        # AnswerString_rel_bb += f"""{object_category_name}-{obj}_[{sub_bb}-{obj_bb}]];"""
        # AnswerString_rel_bb = f"""\{f'Frame {frame_list_idx}': {'sg_triplet': [{subject_category_name}-{sub},{rel},{object_category_name}-{obj}],'sg_location': [{sub_bb},{obj_bb}]}
        # \}"""
        ...
        resp = f"[{subject_category_name}-{sub}:{rel}:{object_category_name}-{obj}_[{sub_bb}:{obj_bb}]];"
...
def getObjectsRelations(vid_rels, vid_data, norm_frames=True, add_frames=True, uniform_sampling_idx=8):
    ...
    for f_obj_idx, f_obj_cnt in enumerate(frames_with_subjobj_rel_cnt):
      ...
      if f"Frame {f_obj_idx}" not in resp_rel:
         resp_rel[f"Frame {f_obj_idx}"] = {
            "tripletes": []
         }
      
      if f"Frame {f_obj_idx}" not in resp_relwith_bb:
         resp_relwith_bb[f"Frame {f_obj_idx}"] = {
            "tripletes": [],
            "bounding_boxes": []
         }
      ...
      for idx, vid_r in enumerate(subj_obj_rel):
        ...
        if subj_obj_rel_entity not in rel_added:
          ...
          # construct_triplet = {
          #    "subject": {
          #       "name": vid_objects_by_id[sub]['category'],
          #       "id": sub
          #    },
          #    "predicate": rel,
          #    "object":{
          #       "name": vid_objects_by_id[obj]['category'],
          #       "id": obj
          #    }
          # }

          # construct_triplet_with_bb = copy.deepcopy(construct_triplet)
          # construct_triplet_with_bb["subject"]["bounding_box"] = sub_bb
          # construct_triplet_with_bb["object"]["bounding_box"] = obj_bb

          # resp_rel[f"Frame {f_obj_idx}"]["tripletes"].append(construct_triplet)
          # resp_relwith_bb[f"Frame {f_obj_idx}"]["tripletes"].append(construct_triplet_with_bb)

          # AnswerString_with_bb += f"[{vid_objects_by_id[sub]['category']}-{sub}-{sub_bb}:{rel}:{vid_objects_by_id[obj]['category']}-{obj}-{obj_bb}]"

          AnswerString_with_bb += f"[{vid_objects_by_id[sub]['category']}-{sub}:{rel}:{vid_objects_by_id[obj]['category']}-{obj}_[{sub_bb}:{obj_bb}]]"
...
if __name__=="__main__":
    ...
    OUTPUT_JSON_DIR = "/lustre/fs1/home/jbhol/dso/gits/OpenPVSG/data/video_llava_annotations_v14_1/"
