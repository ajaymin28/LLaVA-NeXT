...
def addObjectsRelations_bb_instructions(video_path,vid_data,total_frames, subjobj_rel_frames_data, frame_indices,bb_per_object):
  ...
  for frame_list_idx, frame_idx in enumerate(frame_indices):
    ...
    for rel_idx, relation in enumerate(subj_obj_rel):
      ...
      if bb_per_object[subject_category_name]<500 or bb_per_object[object_category_name]<500:
...
if __name__=="__main__":
    ...
    for datasetName in ["vidor", "epic_kitchen", "ego4d"]:

      train_ids = anno["split"][datasetName]["train"] 
      val_ids = anno["split"][datasetName]["val"]
      ...
      OUTPUT_JSON_DIR = f"/lustre/fs1/home/jbhol/dso/gits/OpenPVSG/data/video_llava_annotations_v16_pvsg/"
      JSON_llava_image_tune_validate = f"{OUTPUT_JSON_DIR}/llava_{datasetName}_image_tune_validate.json"
      JSON_llava_image_tune = f"{OUTPUT_JSON_DIR}/llava_{datasetName}_image_tune_.json"
      JSON_videochatgpt_tune = f"{OUTPUT_JSON_DIR}/videochatgpt_{datasetName}_tune_.json"
      JSON_videochatgpt_tune_validate = f"{OUTPUT_JSON_DIR}/videochatgpt_{datasetName}_tune_validate.json"
      ...
      JSON_videochatgpt_bb_tune = f"{OUTPUT_JSON_DIR}/videochatgpt_{datasetName}_tune_bb.json"
      JSON_videochatgpt_bb_tune_validate = f"{OUTPUT_JSON_DIR}/videochatgpt_{datasetName}_tune_bb_validate.json"
      ...
      for ch_idx, chunk_vid_data in enumerate(chunked_list):
        T = threading.Thread(target=prepare_vid_sg_threaded, name=f"Thread{ch_idx+1}", args=(chunk_vid_data,data,True,datasetName,8))
        T.start()
        threads.append(T)
      for th in threads:
        th.join()
