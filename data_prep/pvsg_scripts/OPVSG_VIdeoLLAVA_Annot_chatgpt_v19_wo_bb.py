...
from utils.utilities import getConvBlock, getPromptTemplate, getRandomPrompt, get_pvsg_frame_range_for_annotations
...
from utils.utilities import load_pvsg_annotations
# from utils.utilities import getFramesForObject, create_batch_frames
"""

19_wo_bb

Removed bb instructions
No special tokens for SG only Frameid and SDEnd token
...
def prepare_image_sg(chunk_vid_data_keys,data, norm_bb=True, dataset="vidor", uniform_sampling_idx=8):
  ...
  for vid_data_key in chunk_vid_data_keys:
    ...
    min_frame_idx, max_frame_idx, frames_for_obj = get_pvsg_frame_range_for_annotations(vid_objects, vid_data) # drop frames with no annotations
    ...
    for key_frame_idx, frame_data in frames_where_subjobj_rel_is_present.items():
      ...
      for idx, vid_r in enumerate(subj_obj_rel):
        ...
        if sum(sub_bb)>0 and sum(obj_bb)>0:
          ...
          tripletes_for_current_block += f"[{sub_category}-{sub}"
          tripletes_for_current_block += f":{rel}"
          tripletes_for_current_block += f":{obj_category}-{obj}"
...
def prepare_vid_sg(chunk_vid_data_keys,data, norm_bb=True, dataset="vidor", uniform_sampling_idx=8):
...
if __name__=="__main__":
    ...
    OUTPUT_JSON_DIR = "/lustre/fs1/home/jbhol/dso/gits/OpenPVSG/data/temp/"
    os.makedirs(OUTPUT_JSON_DIR, exist_ok=True)

    data_root = '/lustre/fs1/home/jbhol/dso/gits/OpenPVSG/data/'
    data, anno = load_pvsg_annotations(data_root=data_root,
                                 json_file='pvsg.json')
    ...
    chunked_list_gen = chunk_list(list_=keys, chunk_n=20)
    chunked_list = []
    ...
    for ch_idx, chunk_vid_data in enumerate(chunked_list):
      prepare_vid_sg(chunk_vid_data_keys=chunk_vid_data,norm_bb=True,dataset=dataset_name,uniform_sampling_idx=8)
