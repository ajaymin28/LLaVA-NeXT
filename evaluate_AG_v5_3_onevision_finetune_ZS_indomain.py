...
if __name__=="__main__":
    ...
    # TODO select seed based on the trained model
    ## if model is trained on data generated by seed 324 then use 324.
    Selected_SEED = seed_manager.AG_OBJECT_PREDICATE_PARTIAL_SELECTION_SEEDS[1]
    random.seed(Selected_SEED)
    print("*"*20)
    print(f"USING SEED: {Selected_SEED} FOR EVAL")
    print("*"*20)
    ...
    TempALLObjSet = set(AG_Objects)
    TempAllContactingRel = set(AG_relations["contacting"])
    ObjectsForEval = list(TempALLObjSet.difference(Selected_AG_Objects))
    ObjectsForEval.append("person")
    ContactingRelForEval = list(TempAllContactingRel.difference(Selected_AG_contact_relations))

    INCLUDE_SPATIAL_ATTENTION_REL_FOR_EVAL = True

    ObjectPredicateData = {
        "seed": str(Selected_SEED),
        "selected_objects": list(ObjectsForEval),
        "selected_relations":  {
            # "spatial": [],
            # "spatial": AG_relations["spatial"],
            "contacting": ContactingRelForEval,
            # "attention": [],
            # "attention": AG_relations["attention"],
        },
        "skipped_objects": Selected_AG_Objects,
        "skipped_relations": {
            # "spatial": AG_relations["spatial"],
    ...
    if INCLUDE_SPATIAL_ATTENTION_REL_FOR_EVAL:
        ObjectPredicateData["selected_relations"]["spatial"]= AG_relations["spatial"]
        ObjectPredicateData["selected_relations"]["attention"]= AG_relations["attention"]
        ObjectPredicateData["skipped_relations"]["spatial"]= []
        ObjectPredicateData["skipped_relations"]["attention"]= []
    else:
        ObjectPredicateData["selected_relations"]["spatial"]= []
        ObjectPredicateData["selected_relations"]["attention"]= []
        ObjectPredicateData["skipped_relations"]["spatial"]= AG_relations["spatial"]
        ObjectPredicateData["skipped_relations"]["attention"]= AG_relations["attention"]
        


    AG_Annotations,dataset_meta,video_frame_data = get_AG_annotations_framewise(AG_ANNOTATIONS_DIR=AG_ANNOTATIONS_DIR, 
                                                                                subset=splits[0], EvalKeepData=ObjectPredicateData)
    ...
    #     },
    #     "skipped_objects": Selected_AG_Objects,
    #     "skipped_relations": Selected_AG_contact_relations,
    ...
    AG_Prompt = AG_Prompt.replace("{objects_list}",  ",".join(get_shuffled_list(ObjectsForEval)) )
    AG_Prompt = AG_Prompt.replace("{spatial_relations}", ",".join(get_shuffled_list(AG_relations["spatial"])))
    AG_Prompt = AG_Prompt.replace("{contacting_relations}", ",".join(get_shuffled_list(ContactingRelForEval)))
    ...
    AG_relationsCombined = AG_relations["attention"]+AG_relations["spatial"]+ContactingRelForEval
    ...
    for val_id_idx,AG_Annotation in enumerate(AG_Annotations):
        ...
        for frame_block_index, block_data in enumerate(block_wise_GT_data):
            ...
            for fidx, GT_tripdata in enumerate(Block_GT_triplets_woids):
                ...
                for fgt in frame_GT_triplets:
                    ...
                    # if addIf(triplet=[fgt_s, fgt_p, fgt_o],Objects=ObjectsForEval,relations=ContactingRelForEval):
        ...
        try:
            with open(f"{inference_output_dir}/eval_meta_data.json", "w") as f:
                json.dump(ObjectPredicateData,f, indent=4)
        except Exception as e:
            print(f"error saving : {inference_output_dir}/eval_meta_data.json")
            pass
