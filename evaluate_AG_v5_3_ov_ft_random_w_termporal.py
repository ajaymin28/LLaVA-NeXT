...
from utils.utilities import pre_clean_prediction_data_onevision_FT_v7
...
def parse_args():
    ...
    parser.add_argument("--subset", type=str, default=2000,required=False)
...
if __name__=="__main__":
    ...
    if args.subset is not None and not os.path.exists(args.subset):
        # only number of samples provided
        TotalSamples = len(samples)
        samples = random.sample(samples, int(args.subset))
        print(f"{len(samples)}/{TotalSamples} Samples will be tested")
    ...
    for sample_idx, sample in enumerate(tqdm(samples)):
        ...
        outputs = pre_clean_prediction_data_onevision_FT_v7(outputs_unclean["triplets"])
        ...
        try:
            Block_GT_triplets_woids = remove_ids(Block_GT_Triplets,version="v2_1",remove_indexes=False)
            Block_predicated_triplets_woids = remove_ids(outputs["triplets"],version="v2_1",remove_indexes=False)
        ...
        for fidx, GT_tripdata in enumerate(Block_GT_triplets_woids):
            ...
            try:frame_pred_triplets = Block_predicated_triplets_woids[fidx]
                ...
                print(f"Exception parsing pre triplets: {e}")
                pass
