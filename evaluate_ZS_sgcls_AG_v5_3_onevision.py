...
if __name__=="__main__":
    ...
    with open('/home/jbhol/dso/gits/LLaVA-NeXT/prompts.pkl', 'rb') as handle:
        ...
        defaultPrompt = b["version_14_AG_sgcls"]
    ...
    for val_id_idx,AG_Annotation in enumerate(AG_Annotations):
        ...
        for frame_id, frame_triplets,frame_triplets_bb in video_annotations:
            ...
            if len(frame_indices)>=8:
                ...
                AG_PromptAddedInput += f"""
                Now for the given video and bounding box pairs as follows {str(frame_by_frame_sub_obj_pair_bb)} detect onjects and relations. Answer:
                """
        ...
        if len(frame_indices)>0:
            AG_PromptAddedInput = copy.deepcopy(defaultPrompt)
            AG_PromptAddedInput += f"""
            Now for the provided video and bounding box pairs as follows {str(frame_by_frame_sub_obj_pair_bb)} detect onjects and relations. Answer:
            """
            ## add remaning frames
            block_wise_GT_data.append({
                "frame_idxes": frame_indices,
