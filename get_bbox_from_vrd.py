...
class SceneGraphDetector:
    ...
    @torch.no_grad()
    def predict_bbox_for_scene_graph_vrd(self, scene_graph_dict, data_dir="/home/jbhol/dso/gits/VRDFormer_VRD/data/vidvrd/frames", box_threshold=0.2, text_threshold=0.2, batch_size=16):
        ...
        for video_name, item in scene_graph_dict.items():
            for block_id, block in item.items():
                ...
                for frame_num, frame_triplets in zip(frames[:num_frames], block["triplets"][:num_frames]):
                    ...
                    image_name = f"{video_name}/{str(frame_num+1).zfill(6)}.jpg"
                    ...
                    # if not os.path.exists(image_name):
                    #     print(f"frame not found : {image_name}")
                    #     continue
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_dir', type=str, default="/home/jbhol/dso/gits/VRDFormer_VRD/data/vidvrd/frames")
    ...
    if os.path.isdir(args.result_path):
        ...
        pred_sg = json.load(open(args.result_path, "r"))
    ...
    new_pred_sg = sg_detector.predict_bbox_for_scene_graph_vrd(subset, data_dir=args.data_dir)

    json.dump(new_pred_sg, open(os.path.join(folder_path, f"VRD_inference_val_bbox_{split}-{total}.json"), "w"), indent=4)
